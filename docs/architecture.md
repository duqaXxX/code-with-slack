...
| the same inside a subagent (`parent_tool_use_id` set) | the parent's line shows the subagent's latest call, and becomes a task's line: it keeps a line of its own once it ends, in the foreground or in the background |
