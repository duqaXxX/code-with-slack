...
| `!resume` | Lists the twenty newest sessions of the channel's directory (not of other worktrees), terminal and Slack alike, each with its session id and a **Resume** button; the channel's own is marked `current`. `claude --resume <id>` opens a session in the terminal |
